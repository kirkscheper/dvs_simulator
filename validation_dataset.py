...
def generate_relPose(tProc, xProc, yProc, altitude, folderName = '', path = ''):
	...
	with open(path + '/GT_pose.txt', 'w') as text_file:
		for i in xrange(0, imgCnt + 1):
	 		text_file.write("%i %.6f %.6f %.6f\n" % (i, xProc[i], yProc[i], altitude))
...
alt     = 0.5
folders = []
th = np.linspace(np.pi, -np.pi, t_)
for i in xrange(0,len(th)):
	x_.append(np.cos(th[i]))
	y_.append(np.sin(th[i]))
	z_.append(0)
...
# generate images
imgStart = 0
imgCnt = data.generate_images(1000, folderName = 'val_0') # us
...
generate_relPose(tProc, xProc, yProc, alt, folderName = 'val_0', path = path)
